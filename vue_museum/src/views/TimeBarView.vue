...
import { message } from "ant-design-vue";//测试时注释掉，正式使用时取消注释
...
//     name: `${dynasties[i % dynasties.length]} ${relics[i % relics.length]}`,
...
const timelineWidth = computed(() => {
  const itemWidth = 260; // 每个时间轴项目预估宽度
...
const checkScrollPosition = () => {
  ...
  isScrollLeft.value = container.scrollLeft <= 0;
...
//       title: item.name,
//       date: `${item.dynasty} (${item.dateRange})`,
//     }));
